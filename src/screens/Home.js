...
export default function Home({ navigation }) {
    ...
    const handleSearchTask = (searchPhrase) => {
        ...
        const filteredTodoTasks = todoTasks.filter((task) => task.toLowerCase().includes(searchPhrase.toLowerCase()));
        const filteredCompletedTasks = completedTasks.filter((task) => task.toLowerCase().includes(searchPhrase.toLowerCase()));

        setFilteredTodoTasks(filteredTodoTasks);
        setFilteredCompletedTasks(filteredCompletedTasks);
    };

    // refresh when todoTasks, completedTasks, or search changes
    useEffect(() => {
        ...
        <View style={screen.container}>
            <View style={{ flex: 1 }}>
                ...
                <TaskList
                    ...
                    filteredTodoTasks={filteredTodoTasks}
                    filteredCompletedTasks={filteredCompletedTasks}
                    completeHandler={handleCompleteTask}
